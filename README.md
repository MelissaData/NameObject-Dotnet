# Melissa Data Name Object Windows Net Sample
...
2.  If you already have the latest DQS Release (ZIP), you can find the data file(s) and dll(s) in there
	...
	- Copy all the dll(s) mentioned above into the `MelissaDataNameObjectWindowsNETSample` project folder.

---

#### Configure Target Framework

Depending on your target .NET framework, you may need to configure the powershell script. In order to do this, open up the `MelissaDataNameObjectWindowsNETSample.ps1` for editing, proceed to the bottom of the script where you will find this section of code.
...
- Interactive 
	...
	```
	$ .\MelissaDataNameObjectWindowsNETSample.ps1
	```
    For quiet mode:
    ```
    $ .\MelissaDataNameObjectWindowsNETSample.ps1 -quiet
    ```
- Command Line 

	You can pass a name in ```-name``` parameter and a license string in ```-license``` parameter to test Name object. For example:
	```
    $ .\MelissaDataNameObjectWindowsNETSample.ps1 -name "Ray Melissa"
    $ .\MelissaDataNameObjectWindowsNETSample.ps1 -name "Ray Melissa" -license "<your_license_string>"
    ```
	For quiet mode:
    ```
    $ .\MelissaDataNameObjectWindowsNETSample.ps1 -name "Ray Melissa" -quiet
    $ .\MelissaDataNameObjectWindowsNETSample.ps1 -name "Ray Melissa" -license "<your_license_string>" -quiet
    ```
